  ...
  def __call__(self, inputs, ctx, state, scope=None):
    ...
    with vs.variable_scope(scope or "basic_lstm_cell"):
      ...
      if True:
        ...
        pstate = array_ops.tile(pstate, [49, 1]) 
...
class DropoutWrapper(RNNCell):
  ...
  def __call__(self, inputs, ctx, state, scope=None):
    ...
    if (not isinstance(self._input_keep_prob, float) or
      ...
      ctx = nn_ops.dropout(ctx, self._input_keep_prob, seed=self._seed)
    output, new_state = self._cell(inputs, ctx, state, scope)
    ...
    with vs.variable_scope(scope or "multi_rnn_cell"):
      ...
      for i, cell in enumerate(self._cells):
        with vs.variable_scope("cell_%d" % i):
          ...
          cur_inp, new_state = cell(cur_inp, ctx, cur_state)
